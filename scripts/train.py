...
from lanes.data.sequence_loader import build_test_sequence_loader, build_train_sequence_loader
...
def parse_args():
    parser = argparse.ArgumentParser(description="Unified train entry for baseline/lstm models.")
    ...
    parser.add_argument("--output-dir", type=str, default=None, help="Run root, e.g. outputs")

    parser.add_argument("--sequence-length", type=int, default=None)
    parser.add_argument("--sequence-stride", type=int, default=None)
    parser.add_argument("--sequence-train-list", type=str, default=None)
...
def _build_train_data_loader(cfg, distributed: bool, rank: int, world_size: int):
    if cfg.model_type == "baseline":
        return build_train_loader(cfg, distributed=distributed, rank=rank, world_size=world_size)
    return build_train_sequence_loader(cfg, distributed=distributed, rank=rank, world_size=world_size)
...
def _run_auto_eval(model, cfg, device, output_path: Path, gt_file: str | None, max_batches: int | None, logger):
    ...
    if logger is not None:
        logger.info(
            "auto_eval: model=%s Accuracy=%.6f FP=%.6f FN=%.6f",
            cfg.model_type,
        ...
        logger.info(
            "auto_eval: model=%s frames=%d time_s=%.4f FPS=%.2f pred_file=%s",
            cfg.model_type,
            infer_frames,
            infer_seconds,
            fps,
...
def main():
    ...
    if args.sequence_train_list is not None:
        cfg.sequence_train_list = args.sequence_train_list
    ...
    try:
        ...
        run_name = f"{cfg.model_type}_{run_ts}"
        run_dir = Path(cfg.output_dir) / run_name
        ckpt_path = run_dir / f"{run_name}.pth"
        ...
        if logger is not None:
            logger.info(
                "model=%s device=%s dry_run=%s run_dir=%s distributed=%s rank=%d world_size=%d",
                cfg.model_type,
                device,
                cfg.dry_run,
                run_dir,
            ...
            logger.info(
                "sequence: len=%d stride=%d train_list=%s test_list=%s",
                cfg.sequence_length,
                cfg.sequence_stride,
                cfg.sequence_train_list if cfg.sequence_train_list else cfg.train_list,
                cfg.sequence_test_list if cfg.sequence_test_list else cfg.test_list,
            )

        train_loader, train_sampler = _build_train_data_loader(
            cfg,
        ...
        if logger is not None:
            ...
            logger.info(
                "model_cfg: type=%s hidden=%d lstm_hidden=%d lstm_layers=%d lstm_dropout=%.4f pretrained_backbone=%s",
                cfg.model_type,
                cfg.cls_hidden_features,
        ...
        for epoch in range(cfg.epochs):
            ...
            for step, (images, cls_label) in enumerate(train_loader):
                ...
                if logger is not None:
                    logger.info(
                        "train: model=%s epoch=%d step=%d lr=%.6g total=%.4f cls=%.4f rel=%.4f "
    ...
    if rank == 0 and args.auto_eval:
        if model_to_save is None or run_dir is None or device is None:
